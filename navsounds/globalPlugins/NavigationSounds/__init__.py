...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def event_gainFocus(self, obj, nextHandler):
        ...
        if (cfgSounds):
            STATES = [s for s in obj.states if os.path.exists(
                sounds(s)) == True]
            if len(STATES) > 0:
                for state in STATES:
                    play(state)  # break
            elif os.path.exists(sounds(obj.role)):
                play(obj.role)  # break
        nextHandler()
...
class NavSettingsPanel(SettingsPanel):
    ...
    def makeSettings(self, settingsSizer):
        ...
        self.sou.Set(os.listdir(os.path.join(
            mainPaths, "effects", "navsounds")))
        ...
        self.nab = sHelper.addItem(wx.CheckBox(
            self, label=_("navigation sounds")))
        self.nab.SetValue(config.conf[roleSECTION]["cfgSounds"])
        self.ts = sHelper.addItem(wx.CheckBox(
            self, label=_("keyboard typing sound")))
        self.ts.SetValue(config.conf[roleSECTION]["typing"])
        self.edit = sHelper.addItem(wx.CheckBox(
            self, label=_("enable typing sound in text boxes only")))
        self.edit.SetValue(config.conf[roleSECTION]["edit"])
        self.tlable1 = sHelper.addItem(wx.StaticText(
            self, label=_("select typing sound"), name="tt"))
        self.sou1 = sHelper.addItem(wx.Choice(self, name="tt"))
        self.sou1.Set(os.listdir(os.path.join(
            mainPaths, "effects", "typingsound")))
        self.sou1.SetStringSelection(config.conf[roleSECTION]["type"])
        self.tlable2 = sHelper.addItem(
            wx.StaticText(self, label=_("volume"), name="tt2"))
        self.sou2 = sHelper.addItem(
            wx.SpinCtrl(self, name="tt2", min=0, max=100))
    ...
    def onopen(self, event):
        os.startfile(os.path.join(os.path.abspath(
            os.path.dirname(__file__)), "effects"))
